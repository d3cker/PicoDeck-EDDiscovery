...
#define RX_CAPACITY 32768u
#define JOURNAL_REFRESH_MS 10000u
...
static uint64_t s_last_journal_request_ms;
...
static void send_initial_requests(void) {
    ...
    (void)send_frame(1, status, sizeof(status) - 1);
    (void)send_frame(1, indicator, sizeof(indicator) - 1);
    (void)send_frame(1, journal, sizeof(journal) - 1);
    s_last_journal_request_ms = now_ms();
}

static bool request_recent_journal(void) {
    static const char journal[] =
        "{\"requesttype\":\"journal\",\"start\":-1,\"length\":50}";
    if (!send_frame(1, journal, sizeof(journal) - 1)) return false;
    s_last_journal_request_ms = now_ms();
    return true;
}
    ...
    while (s_rx_length - used >= 2) {
        ...
        if (opcode == 0) {
            if (!s_fragment_opcode ||
                s_message_length + (size_t)length > RX_CAPACITY) {
                return false;
            }
            ...
            if (fin) {
                ...
                if (length > RX_CAPACITY) return false;
            ...
            (void)send_frame(10, payload, (size_t)length);
        } else if (opcode != 10) {
            return false;
        }
...
static err_t connected_cb(void *arg, struct tcp_pcb *pcb, err_t error) {
    ...
    static const char request[] =
        "GET / HTTP/1.1\r\n"
        "Host: 192.168.7.2:6502\r\n"
        "Upgrade: websocket\r\n"
        "Connection: Upgrade\r\n"
        "Sec-WebSocket-Key: UGljb0RlY2tFRERpc3BsYQ==\r\n"
...
void websocket_client_task(bool network_ready) {
    ...
    if (s_state == WS_OPEN) {
        if (now - s_last_rx_ms > 30000) {
            schedule_retry();
        } else if (now - s_last_journal_request_ms > JOURNAL_REFRESH_MS) {
            if (!request_recent_journal()) schedule_retry();
        } else if (now - s_last_ping_ms > 10000) {
            static const char ping[] = "ED";
