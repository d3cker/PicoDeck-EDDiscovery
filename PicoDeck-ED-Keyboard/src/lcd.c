...
void lcd_set_bus_speed(void) {
    spi_set_baudrate(PICODECK_SPI, s_lcd_baud_hz);
}
...
static void lcd_write_command(uint8_t command, const uint8_t *data,
    ...
    for (size_t i = 0; i < length; ++i) {
        ...
        spi_write_blocking(PICODECK_SPI, physical_word, sizeof(physical_word));
        lcd_select(false);
    }
}
...
static uint8_t lcd_read_id(void) {
    ...
    gpio_put(PIN_LCD_DC, 0);
    spi_write_blocking(PICODECK_SPI, &command, 1);
    spi_write_read_blocking(PICODECK_SPI, &dummy, &response, 1);
    lcd_select(false);
    return response;
}
...
void lcd_init(void) {
    ...
    spi_init(PICODECK_SPI, LCD_INIT_BAUD_HZ);
    ...
    /*
     * Waveshare D2U_L2R landscape orientation: 480 by 320, rotated 180
     * degrees so the assembled panel can stand with its USB cable at the top.
     */
...
void lcd_blit_rgb565_be(uint16_t x, uint16_t y, uint16_t width,
    ...
    lcd_select(true);
    gpio_put(PIN_LCD_DC, 1);
    spi_write_blocking(PICODECK_SPI, pixels, byte_count);
    lcd_select(false);
}
...
void lcd_fill(uint16_t color) {
    ...
    for (size_t y = 0; y < LCD_HEIGHT; ++y) {
        spi_write_blocking(PICODECK_SPI, row, sizeof(row));
    }
    lcd_select(false);
}
